...
plot$group <- factor(plot$group , levels = c("Democrat\nMale", "Republican\nMale", "Democrat\nFemale", "Republican\nFemale"))
...
plot %>% 
  ...
  geom_text(aes(y = pct + .0005, label = paste0(pct*100, '%')), position = position_dodge(width = .9), size = 5, family = "font") +
  theme(strip.text = element_text(size = 18)) +
  labs(x = "", y = "Percent of Total Tweets", title = "Mentions of Specific Religious Terms",  caption = "Data: Scraped from Twitter's API (2017 - 2018)") + 
  theme(axis.text.x = element_text(size = 12)) +
  ggsave("D://congress_tweets/images/fig5.png", dpi = 300, width = 10, height =6, type = "cairo-png")
